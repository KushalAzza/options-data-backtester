#!/usr/bin/env python3
"""
Nifty Options End-of-Day Backtest Script

This script implements a specific trading strategy:
- Entry: 15:18:00 (end of day)
- Exit: 15:17:00 next trading day (force exit time)
- On 0DTE (zero days to expiry) entry days: Uses _next_expiry file for entry prices
- Exit prices: Always uses regular file, even if exit date is 0DTE
- Stop loss and target profit: Each leg exits independently when triggered
- VIX threshold: Filters trades based on VIX at entry

IMPORTANT: This script SHORTS (sells) CE and PE options.
- At entry: Sells CE and PE options (receives premium)
- At exit: Buys back CE and PE options (pays premium)
- P&L = (Entry Price - Exit Price) × Lot Size × Lot Multiple

The script:
1. ENTRY (15:18):
   - Checks VIX threshold - skips trade if VIX > threshold
   - Checks if entry date is 0DTE by comparing expiry_date in regular file with entry date
   - If 0DTE, uses the _next_expiry file for entry prices (next weekly expiry options)
   - If NOT 0DTE, uses regular file for entry prices
2. EXIT (next day):
   - Checks prices from 09:18 until 15:17 on exit day
   - Each leg (CE/PE) exits independently when:
     a) Stop loss is hit (price increases by stop_loss_percentage)
     b) Target profit is hit (price decreases by target_percentage)
     c) Force exit at 15:17 if neither triggered
   - ALWAYS uses regular file for exit prices, regardless of whether exit date is 0DTE
3. Independent leg exits: If one leg hits stop loss, the other continues until it hits stop loss, target, or force exit
"""
...
def run_backtest(config: Dict) -> List[Dict]:
    """Run EOD backtest: enter at 15:18, exit next day at 15:17"""
    results = []
    
    # Fixed entry and exit times
    entry_time = "15:18:00"
    exit_time = "15:17:00"  # Force exit time next day
    ...
    while current_date <= end_date:
        ...
        # Get options data for exit day
        # IMPORTANT: Always use regular file for exit, even if exit date is 0DTE
        # The _next_expiry file is ONLY used for entry when entry date is 0DTE
        exit_year = exit_date.year
        exit_options_file = f"{options_data_path}/{exit_year}/nifty_options_{exit_date_str}.json"
        exit_options_data = load_options_data(exit_options_file)
        
        if not exit_options_data:
            print(f"  No options data found for exit date {exit_date_str}")
            current_date += timedelta(days=1)
            continue
        
        # Check stop loss and target profit for each leg independently
        # Each leg can exit at different times
        ...
        if stop_loss_percentage > 0 or target_percentage > 0:
            ce_sl_price, ce_sl_date, ce_sl_time, ce_sl_reason = check_stop_loss_or_target_exit_day(
                exit_options_data, 'CE', ce_strike, date_str, entry_time, exit_date_str, exit_time,
            ...
            if ce_sl_price is not None:
                ...
                ce_exit_reason = ce_sl_reason or "STOP_LOSS"  # Use reason from function
        
        # If CE didn't exit early, use force exit price at 15:17
        if ce_exit_price is None:
            ce_exit_price = get_option_price_closest(exit_options_data, 'CE', ce_strike, exit_time)
            if ce_exit_price is None:
                print(f"  Missing CE exit price for {exit_date_str}")
                current_date += timedelta(days=1)
                continue
        
        # Check PE leg independently
        # If CE exited early, PE continues until it hits stop loss, target, or force exit
        pe_exit_price = None
        ...
        if stop_loss_percentage > 0 or target_percentage > 0:
            pe_sl_price, pe_sl_date, pe_sl_time, pe_sl_reason = check_stop_loss_or_target_exit_day(
                exit_options_data, 'PE', pe_strike, date_str, entry_time, exit_date_str, exit_time,
            ...
            if pe_sl_price is not None:
                ...
                pe_exit_reason = pe_sl_reason or "STOP_LOSS"  # Use reason from function
        
        # If PE didn't exit early, use force exit price at 15:17
        if pe_exit_price is None:
            pe_exit_price = get_option_price_closest(exit_options_data, 'PE', pe_strike, exit_time)
            if pe_exit_price is None:
                print(f"  Missing PE exit price for {exit_date_str}")
                current_date += timedelta(days=1)
                continue
        
        # Calculate P&L for SHORT positions
        # Formula: (Entry Price - Exit Price) × Lot Size × Lot Multiple
        # This represents SELLING options at entry (receive premium) and BUYING BACK at exit (pay premium)
        # Profit when option price decreases, loss when option price increases
        ce_pnl = (ce_entry_price - ce_exit_price) * lot_size * lot_multiple
        pe_pnl = (pe_entry_price - pe_exit_price) * lot_size * lot_multiple
        total_pnl = ce_pnl + pe_pnl
        
        # Get Nifty price at exit (use latest exit time)
        latest_exit_time = ce_exit_time_str if ce_exit_time_str > pe_exit_time_str else pe_exit_time_str
        nifty_exit_price = get_nifty_price_at_time(nifty_data, exit_date_str, latest_exit_time)
        if not nifty_exit_price:
            # Fallback to force exit time
            nifty_exit_price = get_nifty_price_at_time(nifty_data, exit_date_str, exit_time)
        
        # Get VIX at entry and exit
        vix_at_entry = None
        vix_at_exit = None
        if vix_data is not None:
            vix_at_entry = get_vix_price_at_time(vix_data, date_str, entry_time)
            vix_at_exit = get_vix_price_at_time(vix_data, exit_date_str, latest_exit_time)
            if vix_at_exit is None:
                vix_at_exit = get_vix_price_at_time(vix_data, exit_date_str, exit_time)
        ...
        result = {
            ...
            "exit_time": f"{exit_date_str} {exit_time}",
            ...
            "ce_exit_time": f"{exit_date_str} {ce_exit_time_str}",
            ...
            "pe_exit_time": f"{exit_date_str} {pe_exit_time_str}",
        ...
        print(f"  Exit: CE @ {ce_exit_price} at {ce_exit_time_str} ({ce_exit_reason}), PE @ {pe_exit_price} at {pe_exit_time_str} ({pe_exit_reason}), Total P&L: {total_pnl:.2f}")
...
def main():
    ...
    print("=" * 80)
    print("EOD Backtest: Enter at 15:18, Exit next day at 15:17 (or stop loss/target)")
    print("=" * 80)
